...
import { Table, Space, Button, Tooltip, Modal, Form, Input, Popconfirm, Tag, message, Card, Spin, Alert } from 'antd'
...
function LotsCddPage() {
  ...
  const unpublishMutation = useMutation({
    mutationFn: (id: string) => LotCddService.unpublish(id),
    onSuccess: () => {
      message.success('Lot CDD dépublié avec succès')
      queryClient.invalidateQueries({ queryKey: ['lots-cdd'] })
    },
    onError: () => {
      message.error('Erreur lors de la dépublication du lot CDD')
    }
  })

  const transmitMutation = useMutation({
    mutationFn: (id: string) => LotCddService.transmit(id),
    onSuccess: () => {
      message.success('Lot CDD transmis avec succès')
      queryClient.invalidateQueries({ queryKey: ['lots-cdd'] })
    },
    onError: () => {
      message.error('Erreur lors de la transmission du lot CDD')
    }
  })

  const untransmitMutation = useMutation({
    mutationFn: (id: string) => LotCddService.untransmit(id),
    onSuccess: () => {
                ...
                <>
                  ...
                  <Tooltip title="Générer">
                    <Button
                      type="text"
                      size="small"
                      icon={generateMutation.isPending && generateMutation.variables === record._id ? 
                        <Spin size="small" /> : 
