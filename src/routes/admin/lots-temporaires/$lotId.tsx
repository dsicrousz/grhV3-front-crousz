...
export const Route = createFileRoute('/admin/lots-temporaires/$lotId')({
  component: LotTemporaireDetailsPage,
})

function LotTemporaireDetailsPage() {
  const { lotId } = Route.useParams()
  const navigate = Route.useNavigate()

  const { data: lot, isLoading } = useQuery({
    queryKey: ['lot-temporaire', lotId],
    queryFn: () => LotTemporaireService.getOne(lotId),
  })
      ...
      <div className="p-6">
        <Space>
          <Button 
            ...
            onClick={() => navigate({ to: '/admin/lots-temporaires' })}
          >
            Retour
          </Button>
          <Text type="danger">Lot temporaire non trouvé</Text>
        </Space>
      </div>
    )
  }
        ...
        <div className="space-y-6">
          <div>
            <Title level={5} className="mb-4!">
              ...
              Informations du lot temporaire
            </Title>
